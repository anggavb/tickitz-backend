...
func (r *ProfileRepository) GetProfile(ctx context.Context, id int) (model.UserProfile, error) {
	sql := `SELECT COALESCE(u.firstname, '') AS first_name, COALESCE(u.lastname, '') AS last_name, COALESCE(u.phone_number, '') AS phone, COALESCE(u.photo, ''), COALESCE(l.name, '') AS loyalty_tier, u.point, u.email AS point FROM users u LEFT JOIN loyalty_tiers l ON l.id = u.loyalty_tier_id WHERE u.id = $1`
	var profile model.UserProfile
	if err := r.db.QueryRow(ctx, sql, id).Scan(&profile.FirstName, &profile.LastName, &profile.Phone, &profile.Photo, &profile.LoyaltyTier, &profile.Point, &profile.Email); err != nil {
